...
def test_lesson_parse_identity_phase3():
    ...
    # Keys a later phase added additively to `parse_lesson`'s return, which the
    # Phase 3 golden predates. Dropping them outright would weaken this floor
    # into "the old keys still agree", so instead each is asserted to carry
    # exactly its documented default before it is removed from the comparison.
    # That is a stronger statement than the golden alone makes: it says the
    # Phase 3 bank, which carries none of these directives, reads today exactly
    # as it read before they existed. A key whose default changed, or a key
    ...
    for key, default in additive_defaults.items():
        if key not in now:
            fail("parse_lesson no longer returns the additive key %r; every "
                 "return path must carry the same key set" % key)
        if now.pop(key) != default:
            fail("a bank carrying no [%s] directive parsed to a non-default "
                 "%r, so the addition was not additive" % (key, key))
        then.pop(key, None)
...
def test_spec_item_tag_lives_in_shared_fields():
    ...
    types_at = s.find("THE NINE ITEM TYPES")
...
def test_spec_existing_contract_intact():
    ...
    s = itembank.SPEC
    for keep in ("THE NINE ITEM TYPES", "DISTRACTOR ANALYSIS",
