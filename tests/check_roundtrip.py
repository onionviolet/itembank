...
def check_vendor_integrity():
    ...
    # The one allowed keydown is the 6.1 interactive-visual scene's SVG
    # keyboard protocol (arrows step, Enter/Space commit, Escape cancel) --
    # a separate, unrelated handler that main merged in; any other keydown
    # on the page is a hand-rolled editor handler and fails.
    keydowns = re.findall(r"keydown", code, re.IGNORECASE)
    if keydowns and 'svg.addEventListener("keydown"' not in code:
        fail("the page hand-rolls a keydown handler; the editor "
             "configuration must live in the boot script only")
    if len(keydowns) > 1:
        fail("more than the visual-scene keydown handler is hand-rolled; "
             "the editor configuration must live in the boot script only")
