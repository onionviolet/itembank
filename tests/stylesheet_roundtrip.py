...
Five invariants, run over the union of two collectors so a stylesheet built
by a function is covered as well as one held in a constant:
  ...
  3. every `@font-face` url is root-absolute, is a key of the daemon's closed
     font asset map, and the bytes behind it are readable;
  4. against a live daemon, each font url serves 200 `font/woff2`, and the
     same file name under a page-route prefix is a 404;
  5. the daemon's font asset map names exactly the files fonts/MANIFEST.json
     records.
...
# ---- invariant 3: every @font-face url resolves -----------------------------
...
# ---- invariant 4: the live daemon serves them, and only at the real path ----
...
# ---- invariant 5: the map, the manifest and the CSS cannot drift ------------
...
def main():
    ...
    print("ok: stylesheet roundtrip -- %d stylesheets balanced, popover never "
          "suppressed on screen, %d @font-face urls root-absolute, served "
          "200 font/woff2, 404 under a page route, and in step with "
          "fonts/MANIFEST.json" % (len(sheets), len(font_urls(sheets))))
    return 0
