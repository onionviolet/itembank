...
- [ ] **PROTO-01**: Items, sessions, responses, and reports each carry an explicit schema version
...
- [ ] **SURF-01**: One daemon serves every surface on one port: the day view, a sitting, the study loop, reports, settings, and the JSON API
...
- [ ] **SURF-04**: Every capability has both a route and a CLI command, over one runtime
...
- [ ] **DEL-04**: One documented config file holds theme, daily cap, selection weights, auditor autonomy, model backend, and update policy
- [ ] **DEL-05**: `itembank config` prints the settings schema the way `spec` prints the format contract, and an invalid value is rejected with a named error
