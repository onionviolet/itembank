...
| **G5** Evidence honesty | pass on rendering and honesty; the default rollup is provisional | `17B-03 D-06 item 7`: no 17A card primitive carries a fill-state slot for ROLLUP-MAP child cards, so the screen composes from `course_shelf` and `fill_state`. Owner 17A-03. `17B-03 D-06 item 8`: `progress_comprehension_display` omits the determinate-claim `progressbar` role and `aria-valuetext` of `ARIA_CONTRACT`. Owner 17A-03. The default choice itself is owed to Weibao (evidence/17B-03-rollup-choice.md) |
