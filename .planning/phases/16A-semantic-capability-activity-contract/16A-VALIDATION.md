...
| **Measured runtime** | Full suite: **368.1 s** over 78 files, measured 2026-08-28 on Darwin arm64, Python 3.14.6. The two 16A suites within it: `tests/capability_stress_corpus_tracer.py` **9.2 s**, `tests/assessment_authority_adversarial.py` **0.1 s**. Three files in that run are red and all three are pre-existing (`day_roundtrip`, `phase_062_audit`, `retention_ui_roundtrip`); see `16A-TRACER-REPORT.md`. |
...
- **Freeze gate:** no 16A freeze record is written on a red stress-corpus
  ...
  15A-06, and 15B-07).
- **Max feedback latency:** measured 2026-08-28. The slowest per-task quick run is the stress-corpus tracer at **9.2 s**; every other 16A unit file returns in under a second. The full suite at **368.1 s** is the only run above ten seconds, and it is a per-wave rather than a per-task command.
...
**Every one of those counts was met.** Final: `TRACER: 18 passed, 0 skipped, 0
failed` and `ADVERSARIAL: 18 attempted, 18 refused, 0 succeeded`, measured
2026-08-28.

| Plan | Task | Req ID | Behavior | Test Type | Automated Command | Assertion function | Status |
|------|------|--------|----------|-----------|-------------------|--------------------|--------|
| 16A-01 | T1 preconditions | all seven | 14A and 14B landed with the frozen surface 16A was planned against, 13.9 walked, the shipped lesson and evidence surface unmoved, and the additivity baseline recorded | Precondition check, halts by name | `python -c` assertion in the plan's verify block | inline assertions plus `16A-PRECONDITION.md` | passing |
| 16A-01 | T2 D-16A-1 | CAP-01 | The semantic-role representation and the seven role tokens are decided by a human, not adopted | Blocking checkpoint | none (checkpoint) | `16A-DECISIONS.md` heading `## D-16A-1` | recorded under delegation, not answered by Weibao; see that section's Provenance note |
| 16A-01 | T3 D-16A-2 | CAP-02, CAP-03 | The capability registry's module boundary and the media and activity grammar's owner are decided by a human | Blocking checkpoint | none (checkpoint) | `16A-DECISIONS.md` heading `## D-16A-2` | recorded under delegation, not answered by Weibao; see that section's Provenance note |
...
| 16A-08 | T1 direction opt-in plus corpus | A11Y-02 | Explicit `[LESSON-DIR: auto]` gives each text run its own resolution; a lesson declaring nothing renders unchanged | Unit assertions | `python tests/localization_render_check.py`; `python tests/lesson_roundtrip.py` | `localization_render_check.main` | passing |
| 16A-08 | T2 preservation proof | A11Y-02, PORT-01 | Seven cases round-trip byte for byte; no normalization anywhere; a bidi override survives; an invalid direction degrades to a named finding | Tracer scenario with SHA-256 comparisons | `python tests/capability_stress_corpus_tracer.py` | `scenario_localization` | passing |
| 16A-09 | T1 agent and note attackers | ACTIVITY-03 | Two attackers fail all four attacks against the real shipped gates; no mock anywhere | Adversarial suite | `python tests/assessment_authority_adversarial.py` | `attacker_agent`, `attacker_note`, eight `attack_*` | passing |
...
- [x] `tests/assessment_authority_adversarial.py`, ACTIVITY-03's dedicated
...
- [x] All tasks have `<automated>` verify or Wave 0 dependencies. Every task in
  plans 16A-02 through 16A-10 carries an automated verify except the three
  human checkpoints (16A-01 T2, 16A-01 T3, 16A-10 T2), which are checkpoints by
  design and carry a named artifact instead.
- [x] Sampling continuity: no 3 consecutive tasks without automated verify. The
  longest run without one is the two adjacent checkpoints 16A-01 T2 and T3.
- [x] Wave 0 covers all MISSING references. All five Wave 0 items are ticked
  above: the tracer, the adversarial suite, the corpus generator, the
  additivity proof, and `capabilities.py` with its unit assertions.
- [x] No watch-mode flags. Every command in this file is a direct execution.
- [x] Feedback latency measured, not asserted. See the Measured runtime row
