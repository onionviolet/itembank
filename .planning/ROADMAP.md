...
- [ ] **Phase 19A: Course operating surface** - Every course operation the engine already implements gets a route with a CLI twin under `SURFACE_PARITY`, request documents published in `schemas/`. Depends on 14A/14B/14C/15A/15B (all frozen). `19A-CONTEXT.md` records 01 to 03 executed. Continue with treatment binding. Gate: a course created, bound, mapped, treated, audited, and packaged entirely through routes and CLI twins, with the 17B fixture rebuilt that way and compared.
