...
**Freeze waiver, 2026-09-05 (Weibao).** The freeze-before-start ordering is
lifted for every open and future phase. A phase may be planned, executed, and
closed without waiting on a human signature; a phase whose human legs are
unsigned closes as **executed (human legs owed)** rather than `frozen`, and the
owed legs are carried in `STATE.md` until signed. Deterministic gates still
have to actually pass, and an agent still may not sign a human-reserved leg
(17B-CONTEXT D-04). This makes 13.5, 17B, 17C, 18, and all of 19A to 19E
beginnable now, with nothing waiting on anything else.

**Milestone: reach (adopted 2026-09-05).** Full text and order:
`.planning/REACH-MILESTONE.md`. The gap pass found the constraint is reach, not
capability: the course engine is built and has no doors.

- [x] **Phase 19A: Course operating surface** - Every course operation the engine already implements gets a route with a CLI twin under `SURFACE_PARITY`, request documents published in `schemas/`. Depends on 14A/14B/14C/15A/15B (all frozen). Completed 2026-09-07 after the 19A-09 publication gate passed 26 of 26 checks and quick preflight passed. Gate: a course created, bound, mapped, treated, audited, and packaged entirely through routes and CLI twins, with the 17B fixture rebuilt that way and compared.
- [x] **Phase 19B: The agent operation door** - Completed 2026-09-07. The Agent tab, route, and CLI twin share one durable proposal lifecycle. Acceptance creates exactly one journal entry, rejection creates none, and undo restores prior bytes. Focused suites and quick preflight pass. Human screen-reader and visual acceptance remain owed.
- [x] **Phase 19C: Backend on, and the first real director run** - Completed 2026-09-07. The active local Qwen profile reached the director and shared seeding pipeline against disposable real material. The director returned a schema-valid candidate. The weak seed draft failed deterministic checks and accepted zero writes, as required. The recorded diagnostics preserve the operation ID and unchanged bank fingerprint.
- [x] **Phase 19D: Math 1400, through the doors** - Closed for milestone sequencing under the 2026-09-08 scope waiver. The licensed source, objectives, direct reading, shelf, course route, and Agent entry are verified. Live local-AI proposal generation, the new Math 1400 sitting, learner evidence, recommendation, and dependent restore observation remain deferred and unclaimed. Depends on 19A, 19B, 19C. Basis `IL-20260905-05` and `IL-20260908-02`.
- [x] **Phase 19E: The MCP tool table (discharges Phase 999.3)** - Passed 2026-09-07. The generated stdio and HTTP MCP mounts share the existing handlers, and the tool-only course build completes from an empty root through package verification. WebMCP remains V2-INT-03 work after stable browser support.
...
**Goal:** Make the shelf-to-course-to-lesson-to-practice journey structurally clearer through two distinct presentation profiles over one semantic renderer, while separating activity purpose, response format, disclosure, state, and next action without changing runtime authority.
**Requirements:** FLOW-01, FLOW-02, CAP-02, ACTIVITY-01, ACTIVITY-03, A11Y-01, VISUAL-01, VISUAL-02
**Depends on:** Phase 19D Reach closure for execution. Satisfied for sequencing by the 2026-09-08 scope waiver; Phase 20 must not claim the deferred local-AI, sitting, evidence, recommendation, or restore results.
**Plans:** 5 plans across 5 sequential waves

Plans:
- [ ] 20-01-PLAN.md, one representative vertical slice covering inherited audit repairs, semantic hierarchy, profile recipes, item-purpose and response-format separation, transition logic, degraded states, responsive and accessibility evidence, and the human comparison gate.
- [ ] 20-02-PLAN.md, preserve the shipped shelf, next-action, agent, and split modes while comparing Resume, Shelf, Agenda, and Path learner jobs across both profiles, followed by a human retain, combine, or supersede gate.
- [ ] 20-03-PLAN.md, coherent lesson, Study, practice, quiz, and exam hierarchy plus accessible treatment for every existing response type.
- [ ] 20-04-PLAN.md, the complete first-launch-through-return transition graph, offline and interruption recovery, completion semantics, and idempotency evidence.
- [ ] 20-05-PLAN.md, independent profile, home, look, theme, accent, density, contrast, and motion settings; EXT-04 capability adapters; compatible surface adoption; full responsive and accessibility evidence; and closure of every Phase 20-owned audit finding.
