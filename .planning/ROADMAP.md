...
- [ ] **Phase 14C: Source Adapter Registry & Remote Intake (ADDED 2026-08-21)** - One adapter contract and one locator sidecar schema for every source medium: PDF, DOCX, PPTX, EPUB, web capture, transcript, audio and video, OCR. Closes IL-20260815-07, which researched PDF and DOCX on 2026-08-17 and deliberately left the sidecar schema open. Remote sources are in scope (USER-VISION-INBOX 2026-08-20 corrects the learner-owned-files reading as interpretation drift); binding fingerprints a captured local snapshot rather than a URL, so citations stay stable and bound material stays readable offline. Boundary is the existing token-gated daemon JSON API, so an external agent harness is an HTTP client rather than a rewrite. Depends on 14A; blocks the source-binding half of 14B. Context: `.planning/phases/14C-source-adapter-registry/14C-CONTEXT.md`. **Plans:** 8 plans, one wave each (the executor runs sequentially).
  Plans:
  - [ ] 14C-01-PLAN.md - Freeze the locator sidecar schema, build the registry spine, and land the PDF tracer end to end through one route and one CLI command.
  - [ ] 14C-02-PLAN.md - The DOCX adapter including the four OOXML parts python-docx never surfaces, the one hardened zip and XML seam, and the two-file write fault tracer.
  - [ ] 14C-03-PLAN.md - The PPTX adapter with spine order from the presentation part and a real speaker-notes flag, plus its hand-assembled gold fixtures.
  - [ ] 14C-04-PLAN.md - Web capture: hardened fetch, readable extraction with CSS and text-quote anchors, both snapshot storage paths, both bind policies, and the read-time staleness advisory with its route and command.
  - [ ] 14C-05-PLAN.md - Transcript intake for SRT, WebVTT, and plain bracketed timestamps through one timestamp grammar with no third-party dependency.
  - [ ] 14C-06-PLAN.md - The OCR adapter wrapping the existing local-Ollama skill, with an honestly degraded locator the schema enforces.
  - [ ] 14C-07-PLAN.md - EPUB import on stdlib zipfile and xml.etree, with the ebooklib AGPL parking recorded in two durable places.
  - [ ] 14C-08-PLAN.md - Register the ASR adapter as a named refusal, discharge the VENDORED.md obligation with a CI checksum gate, and freeze the phase with a multi-source coverage audit.
