...
- [ ] **Phase 19A: Course operating surface** - Every course operation the engine already implements gets a route with a CLI twin under `SURFACE_PARITY`, request documents published in `schemas/`. Depends on 14A/14B/14C/15A/15B (all frozen), so it is beginnable now. Context written (`19A-CONTEXT.md`, ten decisions pinned, ten plans, one operation family each); plans not yet written. Gate: a course created, bound, mapped, treated, audited, and packaged entirely through routes and CLI twins, with the 17B fixture rebuilt that way and compared.
- [ ] **Phase 19B: The agent operation door** - `surfaces/agent_operation.py` bound to one route and one CLI twin, so an accepted agent proposal is exactly one `journal.commit_operation` with a visible undo, reachable from the Agent tab and from an agent client. Depends on 19A and the shipped 17A-07 state machine. Reach, not authority: it may not settle a mark or release a key. Gate: one proposal accepted from each surface, each one applied journal entry, each undone and re-verified. Context not yet written.
- [ ] **Phase 19C: Backend on, and the first real director run** - A local model profile made active (`model_backend.active` is `""` today) and the director, treatment recommender, and seeding loop run against real material for the first time. Depends on nothing; beginnable now and cheapest to start. Basis `IL-20260905-08` (ROCm 7.2 parity, March 2026). Gate: a recorded run kept verbatim and judged plainly, defects routed to their owning subphase rather than repaired opportunistically. Poor output passes; an unexamined run does not. Context not yet written.
- [ ] **Phase 19D: Math 1400, through the doors** - A real Math 1400 course from an open-licensed algebra source, built with 19A's routes, 19B's agent door, and 19C's backend, and sat at least once. Depends on 19A, 19B, 19C. Basis `IL-20260905-05`; licence read per title at binding time and recorded in rights state. This is the milestone exit, measured in use rather than in a gate record, with the gate machinery kept underneath as the correctness floor. Context not yet written.
- [ ] **Phase 19E: The MCP tool table (discharges Phase 999.3)** - The stdio MCP server of 999.3, generated from `API_ROUTES` as its own success criterion 1 already specifies. Depends on 19A: 999.3 was never blocked on MCP work, only on there being course routes worth exposing. Gate: 999.3's eight criteria unchanged plus one addition, an agent client builds a course through the tool table alone. Precedent `scripts/ocr_mcp.py`; MCP spec revision 2026-07-28. Context not yet written.
