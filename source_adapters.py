...
SOURCE_ADAPTER_ENTRIES = [
    {"name": "markdown", "version": "1.0.0", "handler": _extract_markdown,
     "capability": "Normalize UTF-8 Markdown while retaining line locators.",
     "fallback": "Refuse non-UTF-8 input; convert it to UTF-8 Markdown before importing.",
     "check": "python3 tests/source_adapters_roundtrip.py"},
    {"name": "text", "version": "1.0.0", "handler": _extract_text,
     "capability": "Normalize UTF-8 plain text as Markdown with line locators.",
     "fallback": "Refuse non-UTF-8 input; convert it to UTF-8 plain text before importing.",
     "check": "python3 tests/source_adapters_roundtrip.py"},
    {"name": "pdf", "version": "1.0.0", "handler": _extract_pdf,
     "capability": "Extract born-digital PDF text, tables, and page locators.",
     "fallback": "If PDF text is unavailable, install pdfplumber or supply an authorized text export.",
     "check": "python3 tests/source_adapters_roundtrip.py"},
    {"name": "docx", "version": "1.0.0", "handler": _extract_docx,
     "capability": "Extract WordprocessingML text and supported document parts.",
     "fallback": "If DOCX import is unavailable, install python-docx or supply an authorized text export.",
     "check": "python3 tests/source_adapters_roundtrip.py"},
    {"name": "pptx", "version": "1.0.0", "handler": _extract_pptx,
     "capability": "Extract presentation text and speaker notes in slide order.",
     "fallback": "If PPTX import is unavailable, install python-pptx or supply an authorized text export.",
     "check": "python3 tests/source_adapters_roundtrip.py"},
    {"name": "web", "version": "1.0.0", "handler": _extract_web,
     "capability": "Extract readable blocks from captured HTML with resilient web locators.",
     "fallback": "If readable HTML is unavailable, retain the capture and supply an authorized text or Markdown export.",
     "check": "python3 tests/source_adapters_roundtrip.py"},
    {"name": "transcript", "version": "1.0.0", "handler": _extract_transcript,
     "capability": "Extract timestamped SRT, WebVTT, or bracketed transcript cues.",
     "fallback": "Supply timestamped cues, or import plain text instead and lose cue timing locators.",
     "check": "python3 tests/source_adapters_roundtrip.py"},
    {"name": "ocr", "version": "1.0.0", "handler": _extract_ocr,
     "capability": "Transcribe supported images through the learner's local OCR bridge.",
     "fallback": "If the local OCR bridge is unavailable, transcribe and verify text, then import it as Markdown or text.",
     "check": "python3 tests/source_adapters_roundtrip.py"},
    {"name": "epub", "version": "1.0.0", "handler": _extract_epub,
     "capability": "Extract unencrypted EPUB content in spine order with fragment locators.",
     "fallback": "For encrypted or unreadable EPUBs, supply an authorized unencrypted text or Markdown export.",
     "check": "python3 tests/source_adapters_roundtrip.py"},
    {"name": "asr", "version": "0.0.0", "handler": _extract_asr,
     "capability": "Registered but unimplemented speech-recognition adapter for timed media.",
     "fallback": "Supply a transcript through the transcript adapter; no ASR backend is configured.",
     "check": "python3 tests/source_adapters_roundtrip.py"},
]
